...
# initA=1863800000
# initA=1937100000
initA=27018542328773*8
initReg=copy.copy(reg)
strprg=""
for l in range(len(prg)):
    strprg+=str(prg[l])
strprg="2413750315415530"
maxSame=0
while(not found):
    pnt=0
    ...
    while(pnt<len(prg) and not found):
        ...
        if not jump:
            pnt+=2
        sshort=s.replace(",","")
        strprgshort=strprg[:len(sshort)]
        if sshort!=strprgshort or len(sshort)>len(strprg):
        #     # wrong, jump to next
            pnt=10**9
            if(len(strprgshort)>maxSame):
                print("New record for : {} produce {} expected {}".format(initA, sshort, strprg))
